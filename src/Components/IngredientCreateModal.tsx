...
export const IngredientCreateModal = ({
  onSubmit,
  onClose,
}: IngredientCreateModalProps) => {
  const [name, setName] = useState("");
  const [type, setType] = useState<IngredientType>("VEGETABLES");
  const [allergen, setAllergen] = useState<IngredientAllergen>("NONE");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-main-bg/70 px-4 py-4 backdrop-blur-sm">
      <form
        ...
        /* overflow-hidden here acts as the master mask for the modal itself */
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto scrollbar-themed pr-1">
          ...
          {/* Outer Wrapper handles the rounded border and clips the scrollbar */}
          <div className="mb-4 rounded-2xl border border-ui-border overflow-hidden">
            {/* Inner Scroller handles the overflow and themed scrollbar */}
